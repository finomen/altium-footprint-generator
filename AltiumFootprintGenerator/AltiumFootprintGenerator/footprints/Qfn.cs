...
public class Qfn : FootprintBase
{
    public Qfn()
    {
    ...
    public override bool Equals(IFootprint? other)
    {
        if (other is Qfn qfn)
        {
            ...
            return HPins == qfn.HPins && VPins == qfn.VPins;
        }
        return false;
    }
    ...
    public double ExposedPadWidth { get; set; }
    public double ExposedPadLength { get; set; }
    public string NameBase { get; set; } = "QFN";

    public bool ExposedPad { get; set; } = false;
     
    public override string Name => $"{NameBase}{Pitch:0.00}P{Length:0.0}X{Width:0.0}X{Thickness:0.0}-{Pins}{(ExposedPad ? "EP":"")}";
    ...
    private void RenderPads(PcbComponent comp, SolderGoals goal)
    {
        ...
        {
            var p = new PcbPad(PcbPadTemplate.SmtTop);
            var minDim = Math.Min(ExposedPadWidth, ExposedPadLength);
            p.CornerRadius = (byte)Math.Round(100 *
                Math.Min(GlobalParameters.Cornersize.Limit, GlobalParameters.Cornersize.Relative * minDim) / minDim);
            p.Designator = "EP";
            p.Layer = Layer.TopLayer;
            p.Shape = PcbPadShape.RoundedRectangle;
            p.ShapeTop = PcbPadShape.RoundedRectangle;
            p.Size = CoordPoint.FromMMs(ExposedPadWidth, ExposedPadLength);
            p.StackMode = PcbStackMode.Simple;
            comp.Add(p);
        }
    ...
    private SolderGoals Goals(Density density)
    {
        switch (density)
        {
            ...
            case Density.Least:
                return new SolderGoals(0.2, 0.0, -0, 0.1);
            case Density.Nominal:
                return new SolderGoals(0.3, 0.0, -0, 0.25);
            case Density.Most:
                return new SolderGoals(0.4, 0.0, -0, 0.5);
            default:
